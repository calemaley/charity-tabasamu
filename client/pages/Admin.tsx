...
export default function Admin() {
  ...
  useEffect(() => {
    const existing =
      localStorage.getItem("ADMIN_BASIC") ||
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-charity-orange-100 to-charity-green-100">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
          ...
          <form onSubmit={login} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full px-4 py-2 bg-charity-orange-600 text-white rounded"
            >
              Login
            </button>
          </form>
          <div className="mt-6">
            <div className="text-center text-sm text-charity-neutral-600 mb-2">
              Or use one-time token
            </div>
            <div className="flex gap-2">
              ...
              />
              <button
                className="px-4 py-2 bg-charity-green-600 text-white rounded"
                onClick={saveToken}
              >
                Continue
              </button>
  ...
  const metrics = [
    {
      label: "Hero Slides",
      value: data.hero.images.length,
      color: "bg-charity-orange-600",
    },
    {
      label: "Children",
      value: data.children.length,
      color: "bg-charity-green-600",
    },
    {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          {metrics.map((m) => (
            <div key={m.label} className="bg-white rounded-xl shadow p-4">
              ...
              <div
                className={`mt-2 text-3xl font-bold text-white rounded-lg inline-block px-3 ${m.color}`}
              >
                {m.value}
              </div>
            </div>
        ...
        <Section title="Hero Carousel">
          <HeroEditor
            ...
            onSave={(imgs) => updateSection("hero", { images: imgs })}
          />
        </Section>
        ...
        <Section title="About Section">
          <AboutEditor
            about={data.about}
            onSave={(about) => updateSection("about", about)}
          />
        </Section>
        ...
        <Section title="Help Items">
          <HelpItemsEditor
            items={data.help}
            onSave={(items) => updateSection("help", items)}
          />
        </Section>
        ...
        <Section title="Stats">
          <StatsEditor
            items={data.stats}
            onSave={(items) => updateSection("stats", items)}
          />
        </Section>
        ...
        <Section title="Featured Event">
          <EventEditor
            ...
            onSave={(event) => updateSection("featuredEvent", event)}
          />
        </Section>
        ...
        <Section title="Upcoming Events">
          <EventsListEditor
            ...
            onSave={(list) => updateSection("upcomingEvents", list as any)}
          />
        </Section>

        <Section title="Past Events">
          <EventsListEditor
            list={data.pastEvents}
            onSave={(list) => updateSection("pastEvents", list as any)}
          />
        </Section>

        <Section title="Featured Blog">
          <BlogEditor
            post={data.featuredBlog}
            onSave={(post) => updateSection("featuredBlog", post)}
          />
        </Section>
        ...
        <Section title="Children (Full List)">
          <ChildrenEditor
            ...
            onSave={(children) => updateSection("children", children)}
          />
        </Section>
    ...
    <div>
      ...
      <div className="mt-4 flex gap-2">
        <button
          ...
          onClick={() => setList([...list, { src: "", quote: "" }])}
        >
          Add Slide
        </button>
        <button
...
function AboutEditor({
  ...
  onSave: (about: SiteContent["about"]) => void;
}) {
...
function HelpItemsEditor({
  items,
  onSave,
}: {
  items: HelpItem[];
  onSave: (items: HelpItem[]) => void;
}) {
        ...
        {list.map((it, i) => (
          ...
          >
            <input
              ...
              placeholder="Icon (TrendingUp, Droplets, GraduationCap, Utensils)"
            />
            <input
              className="border rounded px-3 py-2"
              value={it.title}
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex gap-2">
        <button
          className="px-3 py-2 border rounded"
          onClick={() => setList([...list, { number: "0", label: "" }])}
        >
          Add Stat
        </button>
        <button
...
function EventEditor({
  ...
  event: SiteContent["featuredEvent"];
  onSave: (ev: SiteContent["featuredEvent"]) => void;
}) {
      ...
      {Object.entries(form).map(([key, val]) =>
        Array.isArray(val) ? (
          <ArrayEditor
            ...
            title={key}
            values={val as any[]}
            onChange={(arr) => setForm({ ...form, [key]: arr } as any)}
          />
...
function BlogEditor({
  ...
  post: SiteContent["featuredBlog"];
  onSave: (p: SiteContent["featuredBlog"]) => void;
}) {
  ...
  return (
    <div className="space-y-3">
      ...
      <ArrayEditor
        title="content"
        values={form.content}
        onChange={(arr) => setForm({ ...form, content: arr })}
      />
      <input
        className="border rounded px-3 py-2 w-full"
        value={form.image}
...
function EventsListEditor({
  ...
  list: SiteContent["upcomingEvents"];
  onSave: (arr: SiteContent["upcomingEvents"]) => void;
}) {
        ...
        <div key={i} className="border rounded p-4 bg-charity-neutral-50">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <textarea
              className="border rounded px-3 py-2 md:col-span-2"
              rows={2}
              value={ev.fullDescription}
              onChange={(e) => {
                const next = [...events];
                next[i] = { ...next[i], fullDescription: e.target.value };
                setEvents(next);
              }}
              placeholder="Description"
            />
...
function ChildrenEditor({
  ...
  childrenList: SiteContent["children"];
  onSave: (arr: SiteContent["children"]) => void;
}) {
  const [list, setList] = useState(childrenList);
  return (
    <div>
      <p className="text-sm text-charity-neutral-600 mb-3">
        Edit children data. Use the Children page to display details.
      </p>
          ...
          <div key={c.id} className="border rounded p-4 bg-charity-neutral-50">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <ArrayEditor
                ...
                onChange={(arr) => {
                  const next = [...list];
                  next[i] = { ...next[i], needs: arr as string[] };
                  setList(next);
                }}
              />
              <ArrayEditor
                ...
                onChange={(arr) => {
                  const next = [...list];
                  next[i] = { ...next[i], interests: arr as string[] };
                  setList(next);
                }}
              />
              <ArrayEditor
                title="achievements"
                values={c.achievements || []}
                onChange={(arr) => {
                  const next = [...list];
                  next[i] = { ...next[i], achievements: arr as string[] };
                  setList(next);
                }}
              />
              <input
              ...
              <input
                ...
                onChange={(e) => {
                  const next = [...list];
                  next[i] = { ...next[i], dreamJob: e.target.value };
                  setList(next);
                }}
              ...
              <input
                ...
                onChange={(e) => {
                  const next = [...list];
                  next[i] = { ...next[i], monthlyNeed: Number(e.target.value) };
                  setList(next);
                }}
                placeholder="Monthly Need (KES)"
              />
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    const next = [...list];
                    next[i] = { ...next[i], sponsored: e.target.checked };
                    setList(next);
                  }}
                />{" "}
                Sponsored
  ...
  return (
    <div className="border rounded p-3 bg-white/50">
      ...
      <div className="mt-2 flex gap-2">
        ...
        <button
          className="px-3 py-2 bg-charity-orange-600 text-white rounded"
...
function SubscriptionsViewer({
  items,
}: {
  items: SiteContent["subscriptions"];
}) {
  return (
